function [Fcoef1, Fcoef2, f1, f2]=DPco_fexp2(theta1,theta2,or,safr,ngrid,Nwin,Ntr)
...
%Modified by DP in order to accont %for multitrial data!
%        
theta1 = unwrap(theta1);    theta2 = unwrap(theta2);

%--------------------------------------------------------------------------
%DP modification so that there are no derivatives taken between trials
 
%Separate the trials into different columns
theta1=reshape(theta1,Nwin,Ntr);
theta2=reshape(theta2,Nwin,Ntr);
...
%Put again trials to common vectors;
theta1=theta1(:);
theta2=theta2(:);
Dtheta1=Dtheta1(:);
Dtheta2=Dtheta2(:);
