function [ t, x, pout, dxdt, dxdt_ns, cfg] = integrationDelay_NoMex(System,p1,t1,x01,dxdt01,cfg1,plotting)
...
%-dxdt_ns: the first derivative after noise application, a real matrix of time x dimension x trials, or time x dimension (if there is only one trial) structure
...
if NOISE
    ...
    dxdt_ns=zeros(cfg.Nt,cfg.D,cfg.Ntr);
...
if strcmpi(cfg.timestep,'fix')
    ...
    for ii=2:cfg.Nt;
        ...
        if NOISE
            dxdt_ns(:,:,ii-1)=noiseFun(t(ii-1),dxdt(:,:,ii-1),cfg.s_noise);
            x(:,:,ii) = x(:,:,ii-1) + cfg.dt(ii-1)*dxdt_ns(:,:,ii-1);
        else
            x(:,:,ii) = x(:,:,ii-1) + cfg.dt(ii-1)*dxdt(:,:,ii-1);
        end
    end
    ...
    if NOISE
        dxdt_ns(:,:,ii)=noiseFun(t(ii),dxdt(:,:,ii),cfg.s_noise);
    end
    ...
    for jj=1:cfg.Ntr; %Start of trial loop
        ...
        for ii=2:cfg.Nt; %Start of time loop
            ...
            while ct<t(ii) %Start of t(ii-1)->t(ii) time step loop
                ...
                if NOISE
                    cdxdt_ns=noiseFun(ct,dxdt4,cfg.s_noise);
                    cx = cx + cdt*cdxdt_ns;
                else
                    cx = x4;
                end
...
function x = add_noise(~,x,s_noise)%t   
x=x + s_noise.*randn( size(x) );



function x = multi_noise(~,x,s_noise)%t
x_SIZ = size(x);
x=x.*( ones( x_SIZ ) + s_noise.*randn( x_SIZ ) );
