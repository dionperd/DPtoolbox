function [h, z,map]=eegplot(mag,ch,unorm,ch_disp,method,color_res,minVal,maxVal,ColBar)
...
%          
% ch_disp -Optional.Displays the channels that you have selected (white).
% 
...
%Adjust colorbar to include white background
if ~isempty(minVal)
    %mi=min(minVal,min(z(:)));
    mi=minVal;
else
