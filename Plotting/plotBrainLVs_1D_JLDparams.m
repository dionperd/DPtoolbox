function plotBrainLVs_1D_JLDparams(LV,BS,opts,Measure,scales,Color,LineStyle)
...
if isempty(BS) 
    ...
    if strcmpi(opts.method,'BootsRatios')
        ...
        plot(scales,LV,'color',Color,'LineStyle',LineStyle,'LineWidth',2);
        hold on;
        plot(scales(abs(BS)>=opt.th),LV(abs(BS)>=opt.th),'*','MarkerFaceColor',Color,'MarkerEdgeColor',Color,'MarkerSize',2);
    end
