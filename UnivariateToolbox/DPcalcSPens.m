function [P, DOF, logP, PS] = DPcalcSPens(x,logf,winfun,NFFT,Nf,N,slopeInds) %DP: plotting
...
%These are calculated outside the function for computlogation speed:
% N=size(x,1);
% NFFT = 2^nextpow2(N);
% Nf=NFFT/2;
...
FT = fft(x,NFFT);
%get the positive side of the spectrum, without the DC term, and average
%across trials
FT=mean(FT(2:Nf+1,:),2); 
...
P = mean(P,2);%average spectra of trials
