...
function [NE, estimation, nbias,sigma]=DPcalcNEens(x,ncell,approach,base,norm)
...
%Calculate histogram
h=histogram(x,lowerbound, upperbound, ncell); 
...
estimation=estimation+log(count)+logRange - log(ncell);
...
if norm
    ...
    NE = estimation/(logRange/logBase);
end

function result=histogram(x,lower,upper,ncell)
