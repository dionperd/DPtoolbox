...
function [smoothdata,filtwts] = DPeegfilt(data,srate,locutoff,hicutoff,epochframes,filtorder,revfilt,firtype)
...
minfac         = 1; %DP changed it from 3 to 1   % this many (lo)cutoff-freq cycles in filter
...
%------------------------DP change ----------------------------------------    
    if locutoff>0 && hicutoff>0
        fc = (locutoff+hicutoff)/2;
        filtorder = minfac*fix(srate/fc);
    elseif locutoff>0,
...
if filtorder*3 > epochframes,   % Matlab filtfilt() restriction
    ...
    warning('epochframes must be at least 3 times the filtorder.');
    filtorder=floor(epochframes/3)-1; %DP added this line
    fprintf('eegfilt(): filter order is changed to %d. ',filtorder);
end
