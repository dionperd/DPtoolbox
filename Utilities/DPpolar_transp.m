function hpol = DPpolar_transp(theta,rho,distinct_colors,edgealpha,line_style,cax)
...
% 
% if nargs < 1 || nargs > 4
%     error('MATLAB:polar:InvalidInput', 'Requires 1-4 data arguments.')
% elseif nargs == 2 
%     theta = args{1};
%     rho = args{2};
%     if ischar(rho)
...
%     distinct_colors=0;
% else
%     [theta,rho,line_style,distinct_colors] = deal(args{1:3});
% end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%DP%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    ...
    if distinct_colors==1
        ...
        for ii=1:N;
            q(ii) = patchline(xx(:,ii),yy(:,ii),'edgecolor',edgecolor(ii,:),'edgealpha',edgealpha,'parent',cax);
        end
    else
        q = patchline(xx(:),yy(:),'edgealpha',edgealpha,'parent',cax);
    end
else
    ...
    if distinct_colors==1
        ...
        for ii=1:N;
            q(ii) = patchline(xx(:,ii),yy(:,ii),'linestyle',line_style,'edgecolor',edgecolor(ii,:),'edgealpha',edgealpha,'parent',cax);
        end
    else
        q = patchline(xx(:),yy(:),'linestyle',line_style,'edgealpha',edgealpha,'parent',cax);
    end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%DP%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
end
