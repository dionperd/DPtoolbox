...
cfg.timeCalc = cfg.time; %one point every 10, i.e., one calculation at each 40 ms, given that Ts = 4ms or fs=250Hz.
...
for iL=1:Nloops; 
    ...
    for iM = 1:NoutMeasures;
        %...calculate the mean value (excluding possible nan
        %values)
        C.(OutMeasures{iM})(:,:,(Nch-1)*(INDs(iL,2)-1)+INDs(iL,3),INDs(iL,1))=PC{cfg.nm(1),cfg.nm(2)}.(OutMeasures{iM});
