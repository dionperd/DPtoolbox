function DPrunPLSdenis_fMRI_FCD_JLD_perf(w)
...
% % %---------------------A. Stack PLS matrix----------------------------------
% %   Inputs:
% RESfolder='C:\Users\dionperd\Dropbox\Dionysis\PostDocMRS\Results\fMRI_FCD_JLD\Performance\MJLD';
% %-RESfolder: full path to the folder of results .mat files
% PLSmatFolder = ['C:\Users\dionperd\Dropbox\Dionysis\PostDocMRS\Results\fMRI_FCD_JLD\Performance\PLS\',W];
% %-PLSmatFolder: full path to the folder where PLS matrix will be saved
% RESstruct = 'JLD';
% %-RESstruct: name of the result structure (string)
% Conds={'rs1','rs2'};%{'RestEC','Odd_nC','Odd_C'};%'RestEC','Odd_nC','Odd_C'
% %-Conds: cellstring of the names of conditions
% Groups={'YA','OA'};% 'mdo11','mdo12',
...
% Measures = {'alpha','beta','sigma','mu','RMSEcdf', 'peakQ','peakH','peakAQ','peakAH',};%{'P','PS','DOF','logF','H','logV','STD','MSE','MSEn'};%'P','DOF','PS','logV','STD','MSE','logF','H'
% %-Measures: a cellstring of the names of measures to be calculated
% MeasOutsStruct = {[10,1],[10,1],[10,1],[10,1],[10,1],[10,1],[10,1],[10,1],[10,1]};
...
% % logScales = [log(1):(log(20)-log(1))/10:log(20)].'; %logarithmic scaling
% % scales = 1.8*unique(round(exp(logScales)))*w;
% scales =  1.8*[1:10].'*w;
% plotMeasure = @(mean,std,Measure, Colors, Groups, Conds,Ns,scales)plotMeanSTD_1D_GroupsInConds_JLDparams(mean,std,Measure, Colors, PlotGroups, PlotConds, Ns,scales);
% %freqs = [2:2:20];
...
%                     disp(RowIND)
%                     
%                     JLD.alpha = JLD.paramsA(1,:).';
%                     JLD.beta = JLD.paramsA(2,:).';
%                     JLD.sigma = JLD.paramsA(3,:).';
%                     JLD.mu = JLD.paramsA(4,:).';
% %                     JLD.RMSElogpdfQ = JLD.RMSElogpdfQ(:,1);
% %                     JLD.RMSElogpdfH = JLD.RMSElogpdfH(:,1);
%                     
%                     for iM = 1:Nm;%...for each measure...
%                         
...
%-------------------------B. Run PLS--------------------------------------
%Inputs:
OutputFolder = ['C:\Users\dionperd\Dropbox\Dionysis\PostDocMRS\Results\fMRI_FCD_JLD\Performance\PLS\',W];
%-PLSmatFolder: full path to the folder where PLS matrix is saved
PLSmatFolder = ['C:\Users\dionperd\Dropbox\Dionysis\PostDocMRS\Results\fMRI_FCD_JLD\Performance\PLS\BehavPhaseLockMedianMeanCntr\',W];
%-OutputFolder: full path to the folder where PLS output will be stored
%PLSfolder='C:\Users\CoordAgeEEG\Dropbox\DPtoolbox\Statistics\external\plscmd';%TNG
%PLSfolder='C:\Users\dionperd\Dropbox\Dionysis\DPtoolbox\Statistics\external\plscmd'; %laptop
PLSfolder='/Users/dionysos/Dropbox/Dionysis/DPtoolbox/Statistics/external/plscmd'; %MPIB
...
%  opt.stacked_designdata = [[1;1;-1;-1],[1;-1;1;-1]];
...
load('C:\Users\dionperd\Dropbox\Dionysis\PostDocMRS\Results\fMRI_FCD_JLD\Performance\PLS\BehavData.mat');
opt.stacked_behavdata = BehavData(:,2);
Nb = size(opt.stacked_behavdata,2); %number of behavioral measures
Behav = {'Phase Locking Median'}; %cell of strings with behavioral measures'
% labels
...
%     PLSres.(Measures{iM}).var100 = 100*s2/sum(s2);     
   ...
   %B. Brain scores
    ...
    if isfield(PLSres.(Measures{iM}),'boot_result')
        ...
        if any(opt.method == [3,4])
            %lower and upper bootstrap standard errors of task LVs v
            PLSres.(Measures{iM}).vlerr = abs( PLSres.(Measures{iM}).boot_result.llcorr ./ repmat( (PLSres.(Measures{iM}).s).',[Ndof,1] ) - PLSres.(Measures{iM}).v ); %also normalize with s
            PLSres.(Measures{iM}).vuerr = abs( PLSres.(Measures{iM}).boot_result.ulcorr ./ repmat( (PLSres.(Measures{iM}).s).',[Ndof,1] ) - PLSres.(Measures{iM}).v ); %also normalize with s
        else
            %lower and upper bootstrap standard errors of task LVs v
...
Conds = {'Rest Before','Rest After'};%{'Rest','OddnC','OddC'};%
%Behav = {'Age'};
...
% Colors = {rgb('Lime'),rgb('ForestGreen'),rgb('DarkGreen'),...
...
plotBrainLVs = @(BrainLV,BrainLVBootsRatios,BrainLVplotOpts,Measure,scales,Color,LineStyle)plotBrainLVs_1D_JLDparams(BrainLV,BrainLVBootsRatios,BrainLVplotOpts,Measure,scales,Color,LineStyle);
