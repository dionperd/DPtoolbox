function DPplotPLS_PSD_JLDtogetherFun(PLSfolder)
...
h=figure('Name',['Lifespan EEG synchronization dynamics']); %'Visible','off',
...
for iM = 1:PLScfg.Nm;%...for each measure...
    ...
    for iLV = 1:1;
        ...
        for iMM = 1:Nmm;
            subplot(3,2,2*iMM)
            if iMM==1 %if the method is mean-centered...
                title([AllMeasures{iMM},', brain LV_{',num2str(LV),'} (',num2str(thisPLS.var100(LV)),'%, p = ',num2str(thisPLS.perm_result.sprob(LV)),')'])
            else
                title(AllMeasures{iMM})
            end
            hold on;
            plotBrainLVs(sgn*thisPLS.BrainLVs{LV}(:,:,iMM),sgn*thisPLS.BrainLVsBootsRatios{LV}(:,:,iMM),BrainLVplotOpts,AllMeasures{iMM},cfgMeasure);
        end
        hold off;
