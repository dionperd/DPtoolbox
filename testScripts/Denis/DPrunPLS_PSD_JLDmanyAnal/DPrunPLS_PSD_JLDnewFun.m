function DPrunPLS_PSD_JLDnewFun(PLSfolder)

%-------------------------B. Run PLS--------------------------------------
%Inputs:
PLSmatFolder = PLSfolder;
...
cd(PLSmatFolder) 
load('PLSmat.mat')
Nsc0 = length(cfgMeasure.scales);
ind = cfgMeasure.scales>=1.004 & cfgMeasure.scales<=2;
cfgMeasure.scales = cfgMeasure.scales(ind); 
Nsc = length(cfgMeasure.scales);
...
for iM = 1:Nm;%...for each measure...
    ...
    for iG = 1:Ng;
        Ncs = size(PLSmat.(Measures{iM}){iG},1);
        for iCS = 1:Ncs;
            temp = PLSmat.(Measures{iM}){iG}(iCS,:);
            temp = reshape(temp(:), MeasOutsStruct{iM});
            temp = temp(ind,:);
            MeasOutsStruct2{iM} = size(temp);
            tempPLSmat{iG}(iCS,:) = temp(:);
        end
    end
    PLSres.(Measures{iM}) = DPpls_analysis(tempPLSmat, Ns, Nc, opt);
    toc
    
    %Store PLSmat and Msiz(iM)
    PLScfg.(Measures{iM}).PLSmat = PLSmat.(Measures{iM});
    PLSmat = rmfield(PLSmat,Measures{iM});
    PLScfg.(Measures{iM}).MeasOutsStruct = MeasOutsStruct2{iM};
    %PLScfg.(Measures{iM}).MeasOutsStruct(1) = Nsc;
    %PLScfg.(Measures{iM}).MeasOutsStruct(3) = 3;
