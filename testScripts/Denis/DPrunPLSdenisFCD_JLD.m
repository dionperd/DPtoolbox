...
% %---------------------A. Stack PLS matrix----------------------------------
%   Inputs:
RESfolder='C:\data\DATA_Aging_Complexity\Results_FC_FCD_JLD\JLD\ResultsFiles';
%-RESfolder: full path to the folder of results .mat files
PLSmatFolder = 'C:\data\DATA_Aging_Complexity\Results_FC_FCD_JLD\JLD\PLS\Lifespan';
%-PLSmatFolder: full path to the folder where PLS matrix will be saved
RESstruct = 'JLD';
%-RESstruct: name of the result structure (string)
Conds={'RestEC','Odd_nC','Odd_C'};%'RestEC','Odd_nC','Odd_C' 'mdo11','mdo12',
...
MeasOutsStruct = {[76,1],[76,1],[76,1],[76,1],[76, 1]};%{[2048 58],[1 58],[1 58],[47 58],[1 58],[50 58],...
                                                                                        %[50 58],[50 58],[50 58]};
...
PlotConds={'R','OnC','OC'};%;{'R','OnC','OC'}; %'RestEO',
%Colors = {rgb('DodgerBlue'),rgb('ForestGreen'),rgb('Red'), rgb('SteelBlue'),rgb('Teal'),rgb('DarkRed')};
Colors = {...
          rgb('Indigo'), rgb('Magenta'), rgb('DarkOrchid'),...%
          rgb('Maroon'), rgb('Crimson'), rgb('IndianRed'),...%...
          rgb('DarkGreen'), rgb('Lime'), rgb('ForestGreen'),...%
          rgb('DarkBlue'), rgb('Cyan'), rgb('SteelBlue'),...%,
          };
        ...
        for iS=1:Ns(iG); %...for each subject...
            ...
            for iF=1:Nf; %...for each file...
                ...
                if eval(ChooseFileCommand)
                    ...
                    JLD.alpha = JLDcoTr.paramsA(1,:).';
                    JLD.beta = JLDcoTr.paramsA(2,:).';
                    JLD.sigma = JLDcoTr.paramsA(3,:).';
                    JLD.mu = JLDcoTr.paramsA(4,:).';
                    JLD.RMSEcdf = JLDcoTr.RMSElogcdf;
...
for iM=1:Nm;
    ...
    h=plotMeasure(PLSmatStats.(Measures{iM}).mean,PLSmatStats.(Measures{iM}).std,Measures{iM}, Colors, PlotGroups, PlotConds, Ns,cfgJLD.scales*0.004);
...
for iM=1:Nm;
    %h = plotMeasure(PLSmatStats.(Measures{iM}).mean,PLSmatStats.(Measures{iM}).std,Measures{iM},Colors, PlotGroups, PlotConds, Ns,cfg, method);
    %h = plotMeasure(PLSmatStats.(Measures{iM}).mean,PLSmatStats.(Measures{iM}).std,Ns,Ng,Nc,PlotGroups,PlotConds,method,Measures{iM},cfg);
    h=plotMeasure(PLSmatStats.(Measures{iM}).mean,PLSmatStats.(Measures{iM}).std,Measures{iM}, Colors, PlotGroups, PlotConds, [],cfgJLD.scales*0.004);
...
% %-------------------------B. Run PLS--------------------------------------
% %Inputs:
% OutputFolder = 'C:\data\DATA_Aging_Complexity\Results_FC_FCD_JLD\JLD\PLS\RestEO\Aging\MeanCntr';
% %-PLSmatFolder: full path to the folder where PLS matrix is saved
% PLSmatFolder = 'C:\data\DATA_Aging_Complexity\Results_FC_FCD_JLD\JLD\PLS\RestEO\Aging';
