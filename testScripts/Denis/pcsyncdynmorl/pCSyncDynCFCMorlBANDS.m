function pCSyncDynCFCMorlBANDS(inFile,outFile)
...
cfg.domain = 'real'; %otherwise 'real' and a hilbert transform is performed
...
cfg.time = [1:size(data,1)]/cfg.fs;% in seconds, default: cfg.time = [0 N-1]/cfg.fs;
...
for iL=1:Nloops; 
    ...
    %Inputs:
    x(:,1,:) = squeeze(data(:,INDs(iL,1),INDs(iL,3),:));
    x(:,2,:) = squeeze(data(:,INDs(iL,2),INDs(iL,4),:));
    [phi, cfg, method, measInds, Nmeasures, thisfunCommands, Ncomnds, MeasNames, NmeasPmeas] = DPphaseCouplingPrepare(x,cfg);
    clear x;
...
cfg.nm=nm;
cfg.Labels = Labels;
save([outFile,'_CFC_bands.mat'],'C','cfg','-v7.3');
