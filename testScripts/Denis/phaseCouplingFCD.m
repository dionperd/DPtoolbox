...
%Inputs:
filename = '/Users/dionysos/Dropbox/Dionysis/dptoolbox/Figures-Data/DPdata.eeg';% full path, filename and extension of the VA data file
%Optional:
%channels: a vector of channel numbers [1 2 4 etc], or string 'all'
channels = ['all'];
%19 channels from the 10-20 system(?verify this!):
...
cfg.measures = {'CCR'}; %default: 'all' (Notice 'IC' for all Viktor's measures together)
...
cfg.timeCalc = cfg.time(126:end-125); %one point every 10, i.e., one calculation at each 40 ms, given that Ts = 4ms or fs=250Hz.
Ncalc = length(cfg.timeCalc);
cfg.winLen = 1; %the window length in seconds, it will always have an odd number of points
...
OutMeasures = {'CCR'}; %(Sub-)Measures to be saved
NoutMeasures = length(OutMeasures);

tic
%Initialize loop
cfg.freqMode = 'SFC'; %'CFC' for cross-frequency coupling, 'am2ph' for amplitude to phase CFC
cfg.chanMode = 'symmetric'; %or 'directed' for directed measures
[Labels, INDs, fc, nm] = DPconMat2conVecCFG(Nf,Nch,f,hdr.label,cfg.freqMode,cfg.chanMode);
...
for iM = 1:NoutMeasures;
    ...
    %values)
    C.(OutMeasures{iM})=zeros(Ncalc,Ntr,Nf,Nch2);
...
for iL=1:Nloops; 
    ...
    for iM = 1:NoutMeasures;
        ...
        C.(OutMeasures{iM})(:,:,iCh,iF)=PC{cfg.nm(1),cfg.nm(2)}.(OutMeasures{iM});
...
for iM = 1:NoutMeasures;
    %Permute into time x channel pairs x trials x frequencies
    C.(OutMeasures{iM}) = permute(C.(OutMeasures{iM}),[1,3,2,4]);
end
cfg.fc=fc;
cfg.Labels = Labels;
cfg.INDs = INDs;
