function phaseCoupling_start_Between_AB(filenum)

%cd('/Volumes/LIP-Git2/AnaLuisa/Gridmaster_Copy_5August/Export/');
cd('//home/mpib/szymanski/CCR/');
...
[N, Nch, Ntr] = size(data);
cfg.Nch = 42; %%% because its between B
...
cfg.method = 'trial';%this means 'trial' (1 value per trial, no time window)
                         %other: 'trialTime' a sliding window within trial,, 'ensemble' (sliding window across trials)
%cfg.fc = [f(1) f(2)]; %central frequencies of signals, see below

%2. Optional but recommended:
cfg.measures = {'PLV', 'CCR', 'ICI'}; %default: 'all' (Notice 'IC' for all Viktor's measures together)
...
cfg.timeCalc = cfg.time(1:1:end); %one point every 25, i.e., one calculation at each 100 ms, given that Ts = 4ms or fs=250Hz.
Ncalc = length(cfg.timeCalc);
cfg.winLen = 1/cfg.fs; %the window length in seconds, it will always have an odd number of points
...
for iF=1:cfg.Nf; %for all frequencies
    ...
    for iC1=1:21; %for all channel of brain A
        
        for iC2=22:42; %and all channels of brain B
...
for iM = 1:NoutMeasures;
    ...
    %values)
    C.(OutMeasures{iM})=zeros(Ncalc, Nloops);
...
save([filename,'_Between_AB_PC.mat'],'C','cfg');%'Cmean',

disp([filename,'_Between_AB_PC.mat'])
