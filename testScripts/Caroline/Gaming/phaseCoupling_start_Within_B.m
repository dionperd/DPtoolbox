function phaseCoupling_start_Within_B(filenum)
...
[N, Nch, Ntr] = size(data);
cfg.Nch = 21; %%% because its within B
...
cfg.measures = {'PLV', 'PPC', 'CCR'}; %default: 'all' (Notice 'IC' for all Viktor's measures together)
...
for iF=1:cfg.Nf; %for all frequencies
    ...
    for iC1=22:42; %for all channel pairs
        
        for iC2=iC1+1:42; %No self couplings are allowed
...
save([filename,'_Within_B_PC.mat'],'C','cfg');%'Cmean',

disp([filename,'_Within_B_PC.mat'])
