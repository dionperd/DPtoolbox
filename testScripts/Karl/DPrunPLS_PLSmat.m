...
%---------------------A. Stack PLS matrix----------------------------------
%   Inputs:
PLSmatFolder = '/Volumes/tardis/PLS-GT-SFC/';
...
Conds={'DEV-C','DEV-NC','STD-C','STD-NC'}; % data stored in this order
...
Measures = {'C','L','Eloc','Egl'};
%-Measures: a cellstring of the names of measures to be calculated
MeasOutsStruct = {[25 4], [25 4 58], [25 4], [25 4], [25 4 58], [25 4]}; %[25 4 58]
...
for iM=1:Nm;
    ...
    fileName = [PLSmatFolder, filesep, Measures{iM},'_means.fig'];
    saveas(h,fileName);
    close(h);
    clear h;
end
...
for iM=1:Nm;
    ...
    fileName = [PLSmatFolder, filesep, Measures{iM},'_meansSTDerr.fig'];
    saveas(h,fileName);
    close(h);
    clear h;
end
...
OutputFolder='full path to your desired PLS result folder';
...
%%-------------------------C. Plotting PLS---------------------------------
%Inputs:
OutputFolder='full path to your PLS results folder';
%-OutputFolder: full path to the folder where PLS output will be stored
