...
%
%-----------------------------DP addition----------------------------------
%   MCNR: a flag that if 1 and method=2, data are mean centered before
%   Non-Rotated task PLS, just like if method =1 (Mean-Centering Task PLS).
%-----------------------------DP addition----------------------------------
%
...
%		0. Remove group condition means from condition means
...
%		If it is not specified, program will use default value 2. %DP changed this from 0 to 2
...
function result = pls_analysis(datamat_lst, num_subj_lst, k, opt)
   ...
   stacked_behavdata = [];
   is_perm_splithalf = 0;
   meancentering_type = 2; %DP changed this from 0 to 2
   MCNR = 0; %DP added this
   cormode = 0;
   boot_type = 'strat';
   nonrotated_boot = 0;
...
%-----------------------------DP addition----------------------------------      
      if (method==2)
           if isfield(opt,'MCNR')
               MCNR = opt.MCNR;
               if isempty(MCNR) | ~ismember(MCNR,[0,1])
                    error('Field "MCRN" should be 0 or 1');
               end
           end
      end
%-----------------------------DP addition----------------------------------
...
%-----------------------------DP addition----------------------------------
   ...
   [datamatsvd, datamatsvd_unnorm, datamatcorrs_lst, stacked_smeanmat, grand_mean] = ... %DP change
	DPrri_get_covcor(methodCov, stacked_datamat, stacked_behavdata, num_groups, ...
	num_subj_lst, num_cond, bscan, meancentering_type, cormode, ...
	single_cond_lst, 1, num_boot, datamat_reorder, behavdata_reorder);

%------------------------------DP addition---------------------------------
result.datamatsvd = datamatsvd;
result.datamatsvd_unnorm = datamatsvd_unnorm;
result.datamatcorrs_lst = datamatcorrs_lst;
result.stacked_smeanmat = stacked_smeanmat;
...
%------------------------------DP addition---------------------------------      
      %percentage of variance explained by each contrast:
      result.var100 = s.^2 / sum(s.^2) *100;
%------------------------------DP addition---------------------------------
...
%   result.datamat_lst = datamat_lst;
   ...
   if num_perm > 0
	 ...
	 %
         ...
         [datamatsvd, datamatsvd_unnorm] = DPrri_get_covcor(methodCov, ... %DP change
...
%      v_sq = v_sum.^2;
      ...
      for p=1:num_boot
		...
		stacked_smeanmat] = DPrri_get_covcor(methodCov, ...        %DP change
		stacked_datamat, stacked_behavdata, num_groups, ...
		num_subj_lst, num_cond, bscan, meancentering_type, ...
