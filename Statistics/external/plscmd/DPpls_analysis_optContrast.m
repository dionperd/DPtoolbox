...
function result = DPpls_analysis_optContrast(datamat_lst, num_subj_lst, k, opt)
...
%-------------------DP addition to optimize contrasts----------------------
      [r c] = size(crossblock);
      if r <= c
         [u,s,v] = svd(crossblock,0);
      else
         [v,s,u] = svd(crossblock',0);
      end
      
      stacked_designdata = stacked_designdata*u;
