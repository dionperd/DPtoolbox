...
PLSmatFolder = pwd;%'/Users/dionysos/Dropbox/Dionysis/postdocberlin/Publications/BrainSynchronyFEEs/brain_data/DvsS';
...
%%-------------------------B. Run PLS--------------------------------------
%Inputs:
PLSmatFolder ='/Users/dionysos/Dropbox/Dionysis/postdocberlin/Publications/BrainSynchronyFEEs/brain_data/DvsS';
...
for iM = 1:PLScfg.Nm;%...for each measure...
    ...
    for iLV = 1:NlvSign;
       ...
       if any(PLScfg.opt.method==[1,3]) %if the method is mean-centered...
            title([PLScfg.Measures{iM},', LV',num2str(LV),', (',num2str(thisPLS.var100(LV)),'%, p = ',num2str(thisPLS.perm_result.sprob(LV)),'): task LVs'])
        ...
        if any(PLScfg.opt.method==[1,3]) %if the method is mean-centered...
            title([PLScfg.Measures{iM},', LV',num2str(LV),', (',num2str(thisPLS.var100(LV)),'%, p = ',num2str(thisPLS.perm_result.sprob(LV)),'): brain LVs'])
        else
