function [data, hdr, mrk, mrkPtype] = DPreadBV(filename, channels, trials)



[pathstr, name, dummy] = fileparts(filename); 
%older version
...
if exist(hdrFilename,'file')
    ...
    if hdr.nTrials>1
     ...
     else %if there is no segmentation and data is continuous
    ...
    %Read data
    data = DPreadBVeeg(filename, hdr, chanindx,trials);
