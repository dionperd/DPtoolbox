function allinfo = DPreadBVvmrk(filename)
...
MrkTypesWithLabel = {'Stimulus','Response','Marker','Comment'};
...
while line~=-1,
    ...
    % pause
    if ~isempty(line),
        if ~isempty(findstr(line,['Mk',num2str(iM),'='])),
           
            [token,rem] = strtok(line,'=');
            rem(1)=[];
            [token,rem] = strtok(rem,',');
            allinfo.type{iM} = token;
            
            if any(strcmpi(allinfo.type{iM},MrkTypesWithLabel)) 
                [token,rem] = strtok(rem,',');
                allinfo.label{iM} =token;
            else
                allinfo.label{iM} ='';
                %[token,rem] = strtok(rem,',');
            end
            ...
            [token,rem] = strtok(rem,',');
            allinfo.Points(iM) = sscanf(token,'%i');
            
            [token,rem] = strtok(rem,',');
            allinfo.Channel(iM) =sscanf(token,'%i');

            if strcmpi(allinfo.type{iM},'New Segment')
                [token,rem] = strtok(rem,',');
                allinfo.Date{iM} = token;
                %segment=[segment; allinfo.Position(iM)];
%             elseif strcmpi(allinfo.type{iM},'Stimulus')
%                  stimulus=[stimulus; {allinfo.label{iM} allinfo.Position(iM)}];
%             elseif strcmpi(allinfo.type{iM},'Response')
%                  response=[response; {allinfo.label{iM} allinfo.Position(iM)}];
%             elseif strcmpi(allinfo.type{iM},'Time 0')
