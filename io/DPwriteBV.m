function DPwriteBV(dat, filename, hdr, mrk) %, markerfile DP addition
...
if length(size(dat))>2
    %DP: data comes as (time,channels,trials) and becomes (trials,channels,time)
    ntrl  = size(dat,3);
    dat=permute(dat,[3 2 1]); %DP addition

else
    %DP: data comes as (time,channels) and becomes (channels,time)
    dat=permute(dat,[2 1]); %DP addition
end
toc
