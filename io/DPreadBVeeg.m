function [dat] = DPreadBVeeg(filename, hdr, chanindx,trials)%input "trials" added by DP, and inputs begsample, endsample, taken out by DP
...
if strcmpi(hdr.DataFormat, 'binary') && strcmpi(hdr.DataOrientation, 'multiplexed') && any(strcmpi(hdr.BinaryFormat, {'int_16', 'int_32', 'ieee_float_32'}))
  ...
  if isempty(chanindx)
    ...
    if ~strcmpi(hdr.BinaryFormat, 'ieee_float_32') %DP addition
...
%----------------------------DP code---------------------------------------
if hdr.nTrials>1
    dat = reshape(dat,[hdr.SegmentDataPoints, hdr.nTrials, size(dat,2)]);
