function [dat] = DPreadBVeegTF(filename, hdr,channels,trials)%inputs begsample, endsample, taken out by DP

%DP: it reads all channels IF and only IF they are exported in binary
...
% read all channels
%chanindx = []; %DP commented this as unnecessary
...
%------DP: only 'binary', 'multiplexed', 'IEEE_FLOAT_32'-------------------
...
if strcmpi(hdr.DataFormat, 'binary') && strcmpi(hdr.DataOrientation, 'multiplexed') && strcmpi(hdr.BinaryFormat, 'ieee_float_32') %&& any(strcmpi(hdr.BinaryFormat, {'int_16', 'int_32', 'ieee_float_32'}))
...
%   if isempty(chanindx)
    ...
    fseek(fid, NumberOfcolumns*2*(begsample-1), 'cof');
    dat = fread(fid, [NumberOfcolumns, (endsample-begsample+1)], sampletype);
else
    error('The data have to be exported from Vision Analyzer as ''binary'', ''multiplexed'' and ''IEEE_FLOAT_32'' ')
end
...
%----------------------------DP code---------------------------------------
if hdr.ComplexDataPointFactor==2
    dat = reshape(dat,[2, hdr.NumberOfChannels*hdr.Layers, size(dat,2)]);
    dat = dat(1,:,:) + dat(2,:,:)*1i;
    dat=squeeze(dat);
end
dat = reshape(dat,[hdr.Layers, hdr.NumberOfChannels, size(dat,2)]);
dat = permute(dat,[3 1 2]);
dat = reshape(dat,[hdr.SegmentDataPoints, hdr.nTrials, hdr.Layers, hdr.NumberOfChannels ]);
dat = permute(dat,[1 3 4 2]);
   
dat = dat(:,:,channels,trials);
%----------------------------DP code---------------------------------------
