function [TF, param] = DPtfviaCONV(x,t,f,transform,param,plotting) 
...
%Vectorize inputs
t=t(:);
Nt=length(t); %the total number of time points
f=f(:);
f=f.';
Nf=length(f); %the total number of frequency bins
xSIZE=size(x);
if (xSIZE(1)~=Nt)
    error('The first size of x should be equal to the length of the time vector')
end
...
if ischar(transform) && any(strcmpi(transform,methods))
    ...
    if strcmpi(transform,'gabor')
        ...
        %The matlab gaussian window needs parameter: 
        alpha = T/(2*sigma);
        %where T = the total time length/duration:
        GaborWin = gausswin(Nt,alpha);
    ...
    elseif strcmpi(transform,'gaborWavelet')
        ...
        for iF = 1:Nf;
            ...
            %the Gabor wavelet:
            alpha = T/(2*sigma(iF));
            GB = gausswin(Nt,alpha);
            GB = 2* GB/sum(GB);%normalize
            
            Atom(:,iF) = GB.* exp(1i*2*pi*f(iF)*t);
        end
...
%         for iS1=1:Ns1;
%             for iS2=1:Ns2;
%                 TF(:,:,iS1,iS2)=conv2(Atom, x(:,iS1,iS2) ,'same');
%             end
%         end
%         
%     otherwise
