function [cfg] = ft_jakob_singleplotTFR(cfg, data)

% ft_jakob_singleplotTFR plots the time-frequency representation of power of a
% single channel or the average over multiple channels.
%
% Use as
%   ft_jakob_singleplotTFR(cfg,data)
%
...
%
% See also FT_SINGLEPLOTER, FT_MULTIPLOTER, ft_jakob_multiplotTFR, FT_TOPOPLOTER, ft_jakob_topoplotTFR
...
%
% $Id: ft_jakob_singleplotTFR.m 6500 2012-09-20 14:21:42Z roboos $

revision = '$Id: ft_jakob_singleplotTFR.m 6500 2012-09-20 14:21:42Z roboos $';
...
cfg.horizontal_lines     = ft_getopt(cfg, 'horizontal_lines',    []);
cfg.vertical_lines     = ft_getopt(cfg, 'vertical_lines',    []);
cfg.freq_lines     = ft_getopt(cfg, 'freq_lines',    []);
cfg.time_lines     = ft_getopt(cfg, 'time_lines',    []);
cfg.jakob_maskstyle_extra = ft_getopt(cfg, 'jakob_maskstyle_extra',    'no');
...
%%% jakob: get the positions of the vertical and horizontal lines.
if ~isempty(cfg.freq_lines) && isempty(cfg.horizontal_lines)
    for iLine = 1:length(cfg.freq_lines)
        cfg.horizontal_lines(iLine) = nearest(data.freq,cfg.freq_lines(iLine));
        if data.freq(cfg.horizontal_lines(iLine)) < cfg.freq_lines(iLine)
            hcfg.orizontal_lines(iLine) = cfg.horizontal_lines(iLine) +.5;
        elseif data.freq(cfg.horizontal_lines(iLine)) > cfg.freq_lines(iLine)
            cfg.horizontal_lines(iLine) =cfg.horizontal_lines(iLine) -.5;
        end
    end
end

if ~isempty(cfg.time_lines) && isempty(cfg.vertical_lines)
    for iLine = 1:length(cfg.time_lines)
        cfg.vertical_lines(iLine) = nearest(data.time,cfg.time_lines(iLine));
        if data.time(cfg.vertical_lines(iLine)) < cfg.time_lines(iLine)
            cfg.vertical_lines(iLine) = cfg.vertical_lines(iLine) +.5;
        elseif data.time(cfg.vertical_lines(iLine)) > cfg.time_lines(iLine)
            cfg.vertical_lines(iLine) = cfg.vertical_lines(iLine) -.5;
        end
    end
end
...
%change the colormap
if isequal(cfg.jakob_maskstyle_extra, 'yes')
   if length(cfg.jakob_maskstyle_extra_threshold) == 1
      cfg.jakob_maskstyle_extra_threshold(2) = abs(cfg.jakob_maskstyle_extra_threshold(1));
      cfg.jakob_maskstyle_extra_threshold(1) = -abs(cfg.jakob_maskstyle_extra_threshold(1));
   end
   clrmap = colormap;
   %find the colormap rows that correspond to the jakob_maskstyle_extra_threshold value
   b = (cfg.zlim(2)-cfg.zlim(1)) / (length(clrmap)-1);
   a = cfg.zlim(1)-b;
   x = (cfg.jakob_maskstyle_extra_threshold-a)/b;
   x = round(x);
   
   clrmap(x(1):x(2),:) = clrmap(x(1):x(2),:) + (1-clrmap(x(1):x(2),:))*cfg.jakob_maskstyle_transparency;
   colormap(clrmap);
   clear('a','b','c','clrmap');
end
...
if (isfull || haslabelcmb) && shouldPlotCmb
  ...
  if strcmp(cfg.refchannel, 'gui')
    error('coh.refchannel = ''gui'' is not supported at the moment for ft_jakob_singleplotTFR');
%     
...
%     return
  ...
  if ~isfull,
    ...
    elseif strcmp(cfg.directionality, 'ff-fd')
      error('cfg.directionality = ''ff-fd'' is not supported anymore, you have to manually subtract the two before the call to ft_jakob_singleplotTFR');
    elseif strcmp(cfg.directionality, 'fd-ff')
      error('cfg.directionality = ''fd-ff'' is not supported anymore, you have to manually subtract the two before the call to ft_jakob_singleplotTFR');
...
%     mask = squeeze(mask(sellab, ymin:ymax, xmin:xmax));
    mask_jak = squeeze(mask(sellab, ymin:ymax, xmin:xmax));
    mask = nanmean(single(mask_jak), 2);
...
elseif isequal(cfg.masknans,'yes') && ~isempty(cfg.maskparameter)
    ...
    % for jakob_maskstyle
    if isfield(cfg,'jakob_maskstyle') && strcmp(cfg.jakob_maskstyle,'yes')
            mask = nans_mask .* .35;
            mask(mask_jak) = 1;
    else
...
%%%CHANGED to accord for nonlinear Y-axis scalin%%%%%
  set(gca,'YTickMode','manual');
  newtick = linspace(yvector(1),yvector(end),length(yvector));
  set(gca,'YTick',newtick)
  set(gca,'YTickLabel', round(10*yvector)/10)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%jakob: draw horizontal and vertical lines
% if ~isempty(horizontal_lines)
%    vlpos = mean([vdat(ceil(horizontal_lines));vdat(floor(horizontal_lines))])';
%    vlhlims(1) = hdat(1) - mean(diff(hdat))/2;
%    vlhlims(2) = hdat(end) + mean(diff(hdat))/2;
%    line(repmat(vlhlims,[length(horizontal_lines),1])', repmat(vlpos,[1,2])','Color','k','LineStyle',':')
% end
% if ~isempty(vertical_lines)
%    hlpos = mean([hdat(ceil(vertical_lines));hdat(floor(vertical_lines))])';
%    hlhlims(1) = vdat(1) - mean(diff(vdat))/2;
%    hlhlims(2) = vdat(end) + mean(diff(vdat))/2;
%    line(repmat(hlpos,[1,2])',repmat(hlhlims,[length(vertical_lines),1])','Color','k','LineStyle','-.')
% end

if ~isempty(cfg.horizontal_lines)
   hlpos = mean([newtick(ceil(cfg.horizontal_lines));newtick(floor(cfg.horizontal_lines))])';
   hlhlims(1) = xvector(1) - mean(diff(xvector))/2;
   hlhlims(2) = xvector(end) + mean(diff(xvector))/2;
   line(repmat(hlhlims,[length(cfg.horizontal_lines),1])', repmat(hlpos,[1,2])','Color','k','LineStyle',':')
end

if ~isempty(cfg.vertical_lines)
   vlpos = mean([xvector(ceil(cfg.vertical_lines));xvector(floor(cfg.vertical_lines))])';
   vlhlims(1) = newtick(1) - mean(diff(newtick))/2;
   vlhlims(2) = newtick(end) + mean(diff(newtick))/2;
   line(repmat(vlpos,[1,2])',repmat(vlhlims,[length(cfg.vertical_lines),1])','Color','k','LineStyle','-.')
   
   eoslable = 'EoS';
   xtick = get(gca,'XTick');
   [xtick,I] = sort([xtick, cfg.time_lines]);
   set(gca,'XTick',xtick);
   xticklabel = get(gca,'XTickLabel');
   if size(xticklabel,2) > length(eoslable)
      eoslable = [eoslable, repmat(' ',1,size(xticklabel,2)-length(eoslable))];
   elseif bla < length(eoslable)
      xticklabel = [xticklabel, repmat(' ',size(xticklabel,1),length(eoslable)-size(xticklabel,2))];
   end
   xticklabel(I(end),:) = eoslable;
...
%%%CHANGED to accord for nonlinear Y-axis scalin%%%%%
ticky = get(gca,'YTick');
% labsy  = str2num(get(gca,'YTickLabel'));
cfg.ylim = varargin{1}.freq(nearest(ticky,cfg.ylim));

%same for x-axis
% tickx = get(gca,'XTick');
% labsx  = str2num(get(gca,'XTickLabel'));
cfg.xlim = varargin{1}.time(nearest(varargin{1}.time,cfg.xlim));
...
f = figure;
set(f, 'Position', p);
ft_jakob_topoplotTFR(cfg, varargin{:});
