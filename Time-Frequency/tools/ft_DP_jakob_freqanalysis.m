function [freq] = ft_DP_jakob_freqanalysis(cfg, data)

% FT_FREQANALYSIS performs frequency and time-frequency analysis
% on time series data over multiple trials
%   
%   WITH MODIFICATION for Gabortransform & output of evokedpower and
%   phaselocking index (PLI)
%   additional input arguments:
%	cfg.method = 'gabor';
%   cfg.output = 'WP_EP_PLI';      % whole power, evoked power, PLI
%
%   !!! not tested for cfg.keeptrials = 'yes'; or cfg.keeptapers = 'yes'; !!!
%   
%   
%
% Use as
...
cfg = ft_checkconfig(cfg, 'renamedval',  {'method', 'wltconvol', 'wavelet','gabor'});
...
if ~specestflg
  ...
  elseif strcmp(cfg.output,'fourier')
    ...
    csdflg = 0;
    fftflg = 1;
  elseif strcmp(cfg.output,'WP_EP_PLI')
      powflg = 1;
      csdflg = 0;
      fftflg = 0;
      epflg  = 1;
...
%     disp(['processing trial ' num2str(itrial) ': ' num2str(size(data.trial{itrial},2)) ' samples']);
    strg = waitstrg(strg,'processing trial %d: %d samples',itrial,size(data.trial{itrial},2));
    fbopt.i = itrial;
    fbopt.n = ntrials;
    ...
    switch cfg.method
      ...
      case 'gabor'
        ...
        TapBreite   = ft_getopt(cfg, 'TapBreite', .15);

        [spectrum,foi,toi] = ft_jakob_gabor(dat, time, 'timeoi', cfg.toi, options{:},'precision',cfg.precision,'gamma',gamma,'TapBreite',TapBreite);
        
        
        %--------------------------------DP'%addition----------------------
        GaborTransform{itrial} = spectrum;
        phase{itrial} = angle(spectrum);
        %------
    ...
    if itrial == 1
      ...
      if keeprpt == 1 % cfg.keeptrials,'no' &&  cfg.keeptapers,'no'
        ...
        if epflg,  epspctrm      = complex(zeros(nchan,nfoi,ntoi,cfg.precision));    end
        if pliflg, plispctrm     = complex(zeros(nchan,nfoi,ntoi,cfg.precision));    end
      ...
      elseif keeprpt == 2 % cfg.keeptrials,'yes' &&  cfg.keeptapers,'no'
        ...
        if epflg,  epspctrm      = complex(nan+zeros(ntrials,nchan,nfoi,ntoi,cfg.precision),nan+zeros(ntrials,nchan,nfoi,ntoi,cfg.precision));       end
        if pliflg, plispctrm     = complex(nan+zeros(ntrials,nchan,nfoi,ntoi,cfg.precision),nan+zeros(ntrials,nchan,nfoi,ntoi,cfg.precision));       end
      ...
      elseif keeprpt == 4 % cfg.keeptrials,'yes' &&  cfg.keeptapers,'yes'
        ...
        if epflg,  epspctrm      = complex(zeros(ntaptrl,nchan,nfoi,ntoi,cfg.precision));    end
        if pliflg, plispctrm     = complex(zeros(ntaptrl,nchan,nfoi,ntoi,cfg.precision));    end
    ...
    if keeprpt~=4
      ...
      for ifoi = 1:nfoi
        ...
        if epflg 
            epdum = spectrum(acttap,:,foiind(ifoi),acttboi);
        end
        if pliflg
            plidum = spectrum(acttap,:,foiind(ifoi),acttboi) ./ abs(spectrum(acttap,:,foiind(ifoi),acttboi));
        end
        ...
        switch keeprpt
          ...
          case 1 % cfg.keeptrials,'no' &&  cfg.keeptapers,'no'
            ...
            if epflg
              epspctrm(:,ifoi,acttboi)  = epspctrm(:,ifoi,acttboi)  + (reshape(mean(epdum,1),[nchan 1 nacttboi])  ./ ntrials);
            end
            if pliflg
              plispctrm(:,ifoi,acttboi) = plispctrm(:,ifoi,acttboi) + (reshape(mean(plidum,1),[nchan 1 nacttboi]) ./ ntrials);
            end
          case 2 % cfg.keeptrials,'yes' &&  cfg.keeptapers,'no'
            ...
            if epflg
              epspctrm(itrial,:,ifoi,acttboi)  = reshape(mean(epdum,1),[nchan 1 nacttboi]);
              epspctrm(itrial,:,ifoi,~acttboi)  = NaN;
            end
            if pliflg
              plispctrm(itrial,:,ifoi,acttboi) = reshape(mean(plidum,1),[nchan 1 nacttboi]);
              plispctrm(itrial,:,ifoi,~acttboi) = NaN;
            end
      ...
      if epflg
        epspctrm(currrptind,:,:) = spectrum;
      end
      if pliflg
        plispctrm = spectrum ./ abs(spectrum);
      end
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     if epflg
%       epspctrm = abs(epspctrm); %before epspctrm was a summing up of the complex spectra -> by taking the abs() now, phase-relations between trials are taken into account (in contrast to wholepower (=powspectr))
%       epspctrm = epspctrm.*ntrials;
%       epspctrm = epspctrm./trlcnt(ones(size(powspctrm,1),1),:,:);
%       epspctrm(nanmask(ones(size(epspctrm,1),1),:,:)) = nan;
%     end
%     if pliflg
%       plispctrm = abs(plispctrm); %%before plispctrm was a summing up of the NORMALIZED complex spectra -> by taking the abs() now, constancy of phase between trials is calculated
%       plispctrm = plispctrm.*ntrials;
%       plispctrm = plispctrm./trlcnt(ones(size(powspctrm,1),1),:,:);
%       plispctrm(nanmask(ones(size(plispctrm,1),1),:,:)) = nan;
%     end
  end
  ...
  freq = [];
  %--------------------------------DP' addition----------------------------
  freq.complexTransform = GaborTransform; clear GaborTransform;
  freq.phase = phase; clear phase;
  %------------------------------------------------------------------------
  ...
  if epflg
    if ~isempty(hasdc_nyq)
      if keeprpt>1
        epspctrm(:,:,hasdc_nyq,:) = epspctrm(:,:,hasdc_nyq,:)./2;
      else
        epspctrm(:,hasdc_nyq,:) = epspctrm(:,hasdc_nyq,:)./2;
      end
    end
    freq.epspctrm = abs(epspctrm).^2;
  end
  if pliflg
    if ~isempty(hasdc_nyq)
      if keeprpt>1
        plispctrm(:,:,hasdc_nyq,:) = plispctrm(:,:,hasdc_nyq,:)./2;
      else
        plispctrm(:,hasdc_nyq,:) = plispctrm(:,hasdc_nyq,:)./2;
      end
    end
    freq.plispctrm = abs(plispctrm);
  end
  if strcmp(cfg.calcdof,'yes');
    freq.dof = 2 .* dof;
  end;
